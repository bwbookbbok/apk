...
//            ->update(['total_count' => $total, 'limit_count' => $limit]);
        ...
        if (!is_array($bid_result) || !array_key_exists('data', $bid_result) || !$bid_result['data']) {
            //创建包名
            $platform = 'IOS';
...
////        $supurl = $this->alupload($param);
        ...
        //$ml =  'isign -c '.$absolute_path.'public/spcer/liufuqiang/certificate.pem -k /www/wwwroot/www.xxx/public/spcer/liufuqiang/key.pem -p "'.$files.'"  -o /www/wwwroot/xxxx/public/testIpa/'.$udid.'resigned.ipa "'.$ipa.'" 2>&1'.'<br/>';
        //
